...
describe("get_mini_league", () => {
  it("returns the Rails JSON body as a text block on success", async () => {
    const body = {
      league_id: 987654,
      league_name: "Office League",
      standings: [
        { rank: 1, team_id: 1234567, team_name: "Fantasy Wanderers", total_points: 412 },
        { rank: 2, team_id: 7654321, team_name: "Rival FC", total_points: 398 },
      ],
      stale: false,
    };
  ...
  it.each([
    [404, "That league ID doesn't exist or isn't public"],
    ...
    const result = await client.callTool({ name: "get_mini_league", arguments: { league_id: "999999999" } });

    expect(result).toEqual({ content: [{ type: "text", text: message }], isError: true });
  });

  it("surfaces a missing league_id as a clean isError tool result, not an unhandled throw", async () => {
    ...
    const result = await client.callTool({ name: "get_mini_league", arguments: {} });
